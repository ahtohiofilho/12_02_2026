...
class UnitStats:
    ...
    # 0 = só o próprio tile
    # 1 = tile próprio + adjacentes (aeronaves / navios de combate)
...
UNIT_STATS: dict[str, UnitStats] = {
    ...
    "support_vehicle":      UnitStats("Veículo de Suporte",            UnitCategory.LAND,   4.0,  2.0, 3, "support_vehicle",
                                      can_transport=True),
    "atgm_team":            UnitStats("Equipe ATGM",                   UnitCategory.LAND,   5.0,  3.0, 2, "atgm"),
    "sp_artillery":         UnitStats("Artilharia Autopropulsada",     UnitCategory.LAND,   8.0,  5.5, 1, "sp_artillery"),
    "shorad":               UnitStats("Defesa AA Curto Alcance",       UnitCategory.LAND,   6.0,  3.5, 2, "shorad"),

    # ------------------------------------------------------------------ #
    # AIR                                                                  #
    # ------------------------------------------------------------------ #
    "fighter":              UnitStats("Caça",                          UnitCategory.AIR,    8.0,  4.5, 8, "fighter",
                                      vision_range=1),
    "strike_aircraft":      UnitStats("Aeronave de Ataque",            UnitCategory.AIR,   14.0,  7.0, 6, "strike",
                                      vision_range=1),
    "ucav":                 UnitStats("Drone de Ataque (UCAV)",        UnitCategory.AIR,    7.0,  3.5, 7, "ucav",
                                      vision_range=1),
    "transport_aircraft":   UnitStats("Aeronave de Transporte",        UnitCategory.AIR,    8.0,  2.5, 6, "transport_aircraft",
                                      can_transport=True),

    # ------------------------------------------------------------------ #
    # NAVAL                                                                #
    # ------------------------------------------------------------------ #
    "frigate":              UnitStats("Fragata/Corveta",               UnitCategory.NAVAL, 18.0, 10.0, 3, "frigate",
                                      vision_range=1),
    "destroyer":            UnitStats("Destroyer",                     UnitCategory.NAVAL, 28.0, 16.0, 3, "destroyer",
                                      vision_range=1),
    "aircraft_carrier":     UnitStats("Porta-Aviões",                  UnitCategory.NAVAL, 50.0, 12.0, 2, "aircraft_carrier",
                                      can_transport=True, vision_range=1),
    "submarine":            UnitStats("Submarino",                     UnitCategory.NAVAL, 24.0, 14.0, 2, "submarine"),
    "amphibious_ship":      UnitStats("Navio Anfíbio",                 UnitCategory.NAVAL, 12.0,  5.0, 3, "amphibious_ship",
                                      can_transport=True),

    # ------------------------------------------------------------------ #
    # CIVILIAN                                                             #
    # ------------------------------------------------------------------ #
    "worker":               UnitStats("Trabalhador",                   UnitCategory.CIVILIAN, 1.0, 0.2, 3, "worker",
                                      is_non_combat=True),
}
